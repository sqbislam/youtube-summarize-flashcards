        ...
        <BounceCard className="col-span-12 md:col-span-4">
          ...
          <div className="absolute bottom-0 left-4 right-4 top-32 translate-y-8 rounded-t-2xl bg-gradient-to-br from-red-950 to-red-700 p-4 transition-transform duration-[250ms] group-hover:translate-y-4 group-hover:rotate-[2deg]">
            <span className="block text-center font-semibold text-red-50">
              Generate summary of videos with the click of a button
            </span>
          </div>
        </BounceCard>
...
const BounceCard = ({
  ...
  return (
    <motion.div
      ...
      className={`group relative min-h-[300px] cursor-pointer overflow-hidden rounded-2xl bg-secondary dark:text-slate-400 p-8 ${className}`}
    >
      {children}
    </motion.div>
  );
